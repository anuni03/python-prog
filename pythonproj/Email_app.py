...
def validate_login():
    ...
    if(email_text=="") or (pass_text==""):
        ...
        EMAIL_REGEX=re.compile(r"[^@\s]+@[^@\s]+\.[a-zA_Z0-9]+$")
...
def validate_message():
    ...
    if(email_text=="")or(sub_text=="")or(msg_text==""):
        label9.grid()
        label9['text']="Fill all the Places"
        root.after(10,root.grid)
        return False
    else:
        EMAIL_REGEX=re.compile(r"[^@\s]+@[^@\s]+\.[a-zA_Z0-9]+$")
...
label4=Label(f2,width=20,bg="cyan",fg="red",text="logged in successfully!",font=('Calibri 12 bold'))
